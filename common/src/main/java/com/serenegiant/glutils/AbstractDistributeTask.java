...
public abstract class AbstractDistributeTask implements IMirror {
    ...
    private final SparseIntArray mRemoveSurfaceClearColor = new SparseIntArray();
     ...
     * コンストラクタ
     */
    protected AbstractDistributeTask(final int width, final int height, @Nullable GLDrawer2D.DrawerFactory factory) {
     ...
     */
    @AnyThread
    public void requestFrame(final boolean isOES, final int texId, @NonNull @Size(min = 16) final float[] texMatrix) {
     ...
     */
    @AnyThread
    public void addSurface(final int id, final Object surface) throws IllegalStateException, IllegalArgumentException {
        addSurface(id, surface, null);
    }

    @AnyThread
    public void addSurface(final int id, final Object surface, @Nullable final Fraction maxFps)
        throws IllegalStateException, IllegalArgumentException {
        addSurface(id, surface, maxFps, 0);
    }
    ...
    @AnyThread
    public void addSurface(final int id, final Object surface, @Nullable final Fraction maxFps,
        final int removeClearColor) throws IllegalStateException, IllegalArgumentException {
        ...
        synchronized (mTargets) {
            ...
            if (target == null) {
                mRemoveSurfaceClearColor.append(id, removeClearColor);
            }
        }
     ...
     */
    @AnyThread
    public void clearSurface(final int id, final int color) throws IllegalStateException {
     ...
     */
    @AnyThread
    public void clearSurfaceAll(final int color) throws IllegalStateException {
    ...
    public void resize(final int width, final int height) throws IllegalStateException {
        ...
        checkFinished();
        if (((width > 0) && (height > 0)) && ((mVideoWidth != width) || (mVideoHeight != height))) {
    ...
    @WorkerThread
    protected Object handleRequest(final int request, final int arg1, final int arg2, final Object obj) {
     ...
     */
    @WorkerThread
    private void handleDraw(final boolean isOES, final int texId, @NonNull @Size(min = 16) final float[] texMatrix) {
     ...
     */
    @WorkerThread
    protected void handleDrawTargets(final boolean isOES, final int texId,
    ...
    @WorkerThread
    @NonNull
    private RendererTarget createRendererTarget(@NonNull final EGLBase egl, final int id, @NonNull final Object surface,
    ...
    private void handleRemoveSurface(final int id) {
        ...
        if (target != null) {
            mTargets.remove(id);
            final int clearColor = mRemoveSurfaceClearColor.get(id, -2);

            if (clearColor > -2) {
                mRemoveSurfaceClearColor.delete(id);
            }

            if (target.isValid() && clearColor >= 0) {
                target.clear(clearColor);    // XXX 黒で塗りつぶし, 色指定できるようにする? できます！
            }
            target.release();
        }
        checkTarget();
     ...
     */
    @WorkerThread
    private void handleSetMvp(final int id, final int offset, @NonNull @Size(min = 16) final float[] mvp) {
    ...
    private static class TargetSurface {
        ...
        public String toString() {
            return "TargetSurface{" + "id=" + id + ", surface=" + surface + ", maxFps=" + maxFps + '}';
        }
